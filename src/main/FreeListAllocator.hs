module FreeListAllocator 
  ( -- * Types
    Address, Size, Block(..), AllocatorState(..)
    -- * Functions
  , initAllocator, allocate, deallocate
  ) where
...
data Block = Block {
        bAddr :: Address, -- ^ Starting address.
        bSize :: Size     -- ^ Size of the block.
} deriving (Show, Eq)

-- | Current state of the free memory pool.
data AllocatorState = AllocatorState {
        freeBlocks :: [Block] -- ^ List of available blocks (ordered by address).
} deriving (Show, Eq)
